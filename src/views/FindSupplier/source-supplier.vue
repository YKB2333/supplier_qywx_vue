<template>
  <div class="page" id="source-supplier-scroll">
          ...
          >
            <div v-if="$route.query.type === 'JP' && currentTitleIndex === 0">
              <div
                ...
                :class="{ 'van-hairline--top': !idx }"
                v-for="(item, idx) in list"
                :key="item.id"
                @click="$router.push({path: '/JapanNewsDetails', query: {id: item.id}})"
              >
                <img
                  :src="formatImageType(item) || require('@/assets/images/logo.png')"
                  width="109"
                  height="109"
                  class="object-contain"
                />
                <div class="flex-1 ml12 col" style="overflow: hidden;justify-content: center;">
                  <p class="fs16 l-h21 mb10 mt2 ellipsis2">{{ item.title }}</p>
                  <p v-if="item.supplierName" class="ellipsis">
                    <span
                      class="ptb4 plr6 fs14 fw500"
                      style=" line-height: 14px;background-color:rgba(87, 180, 180, .1);border-radius: 1px;"
                    >
                      <span style="color: #57B4B4;">{{ item.supplierName }}</span>
                    </span>
                  </p>
                  <p class="fs12 fw400 l-h17 mt14 row space-between" style="color: #999999;">
                    {{ format(item.createTimestamp) }}
                    <span
                      v-if="userMobile === item.creatorMobile && item.type === '1'"
                      class="mr8 pointer"
                      @click.stop="onDelInfo(item.id, idx)"
                    >删除</span>
                  </p>
                </div>
              </div>
            ...
            <div v-else>
              <van-grid :column-num="2" border square>
                <van-grid-item
                  v-for="(item, idx) in list"
                  :key="idx"
                  @click="$router.push({path:'/detailsmessage',query: {id: item.id}})"
                >
                  <div slot="icon" class="w64 h64 relative">
                    <img
                      :src="item.logo || require('@/assets/images/logo.png')"
                      class="pos-center max-w-h-full"
                    />
                  </div>
    ...
    <!-- 发帖 -->
    <div v-if="enableSend && currentTitleIndex === 0" class="post-box" @click.stop="onJumpPost">
      <img :src="require('@/assets/images/icon_ft.png')" class="w-h60" />
    </div>
  </div>
</template>
...
import { searchPage, searchInfoPage, enableSend, delInfo } from '@/api/supplier'
...
export default {
  ...
  mixins: [new savePosition('source-supplier-scroll')],
  data() {
    return {
      role,
      supTypes: {},
  ...
  methods: {
    ...
    formatImageType(item) {
      return item.cmsInfoAttas && item.cmsInfoAttas[0] && item.cmsInfoAttas[0].attaType  === '2' ? item.cmsInfoAttas[0].snapshot : item.titleImage
    },
    ...
    //       return
    //     }
    //     let arr = deepClone(res.records).map(item => item.loading = true)
    ...
    fetchVendorBrandData() {
      return cmsSelectPage({
        ...
        pageSize: 1,
        infoCategoryId:
          this.$route.query.type === 'JP'
            ? cmsCategoryId.JPBrand
            : cmsCategoryId.vendorBrand,
        status: '1',
    ...
    getSwiperInit() {
      setTimeout(() => {
        let galleryThumbs = new Swiper('.gallery-thumbs', {
          ...
          onSlideChangeEnd: galleryThumbs => {
        ...
        if (
          this.$route.query.type === 'JP' &&
          this.$route.query.sourceType !== 'market'
        ) {
